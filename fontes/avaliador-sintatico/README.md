...
No diretório `dialetos` temos a implementação de avaliadores sintáticos de dialetos de Delégua, que começam sendo implementados neste núcleo. Conforme a necessidade, são separados em diferentes pacotes do ecossistema NPM. Outros dialetos, como [VisuAlg](https://github.com/DesignLiquido/visualg), [Portugol Studio](https://github.com/DesignLiquido/portugol-studio), [Mapler](https://github.com/DesignLiquido/mapler), [Potigol](https://github.com/DesignLiquido/potigol) e [BIRL](https://github.com/DesignLiquido/birl), começaram sendo implementados aqui, mas maturaram e foram separados. Os dialetos que ainda estão aqui podem um dia ser separados também, mas isso requer uma certa evolução nas suas particularidades, junto com uma real necessidade para essa separação.

## Micro Avaliador Sintático

Há algumas situações em que a avaliação sintática de uma linguagem precisa ser feita num âmbito reduzido, como por exemplo em interpolações de texto (como no analisador sintático de Delégua), ou em verificação preguiçosa de parâmetros de funções (ver [avaliador sintático de Potigol](https://github.com/DesignLiquido/potigol/tree/principal/fontes/avaliador-sintatico) para um exemplo). Não faz sentido em uma interpolação de texto termos funcionalidades totais de Delégua, como uma declaração de classe, de tipo ou de função, mas faz sentido avaliarmos expressões menores, como operações matemáticas ou de texto. 
