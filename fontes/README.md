...
* `analisador-documentario`: fontes que implementam a análise documentária de Delégua e demais dialetos contidos neste projeto. A análise documentária é a etapa de avaliação de código que tem como objetivo extrair informações de documentação do código, como comentários, anotações, e outras informações que não são essenciais para a execução do código, mas que podem ser úteis para o programador. A análise documentária é uma etapa opcional, mas pode ser muito útil para gerar documentação automática do código, ou para fornecer informações adicionais ao programador durante a depuração de código;
