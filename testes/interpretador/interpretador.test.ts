    ...
    describe('interpretar()', () => {
        ...
        describe('Cenários de sucesso', () => {
            ...
            describe('Escolha - Caso', () => {
                ...
                it('Deve parar o loop quando encontrar o primeiro "caso" que satisfaz', async () => {
                    const retornoLexador = lexador.mapear([
                        'var dia = "segunda"',
                        'escolha (dia) {',
                        '   caso "segunda":',
            ...
            it('Deve respeitar valores iniciados por operador unário quando usado "retorna"', async () => {
                const retornoLexador = lexador.mapear([
                    'funcao negativo() {',
                    '    retorna -42 ',
                    '}',
            ...
            it('Deve inferir tipo lógico no retorno de função com comparação', async () => {
                const codigo = [
                    'funcao ola(nome) {',
                    '    escreva("Olá, eu me chamo ${nome}")',
                    '    retorna nome != 10',
                    '}',
                    'escreva(tipo de ola("Victor"))'
                ];
                const retornoLexador = lexador.mapear(codigo, -1);
                const retornoAvaliadorSintatico = await avaliadorSintatico.analisar(retornoLexador, -1);
                const retornoInterpretador = await interpretador.interpretar(retornoAvaliadorSintatico.declaracoes);
        ...
        describe('tipo de', () => {
            ...
            it('tipo de literal número real', async () => {
                const codigo = ['escreva(tipo de 3.141592)'];
                const retornoLexador = lexador.mapear(codigo, -1);
                const retornoAvaliadorSintatico = await avaliadorSintatico.analisar(retornoLexador, -1);
                const retornoInterpretador = await interpretador.interpretar(retornoAvaliadorSintatico.declaracoes);
                expect(retornoInterpretador.erros).toHaveLength(0);
                expect(_saidas[0]).toContain('real');
            });
