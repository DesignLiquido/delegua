    ...
    describe('interpretar()', () => {
        ...
        describe('Cenários de sucesso', () => {
            ...
            describe('Decoradores', () => {
                ...
                it('Suporta o uso de Decoradores com parâmetros', async () => {
                    const codigo = [
                        'função de decorador meu_decorador(decorado, nome):',
                        '   retorna funcao():',
                        '       escreva(f"Olá, {nome}")',
                        '       decorado()',
                        '       escreva(f"Tchau, {nome}")',
                        '',
                        '@meu_decorador("Victor")',
                        'função ola_mundo():',
                        '   escreva("Olá, Mundo!")',
                        '',
                        'ola_mundo()'
            ...
            it('Deve parar o loop "escolha" quando encontrar o primeiro "caso" que satisfaz', async () => {
                const retornoLexador = lexador.mapear([
                    ...
                    'escolha (dia):',
                    '   caso "segunda":',
                    '       escreva("Início da semana")',
                    '   caso "segunda":',
                    '       escreva("Isso era pra ser executado de novo?")',
                    '   caso "sexta":',
            ...
            it('Deve inferir tipo lógico no retorno de função com comparação', async () => {
                const codigo = [
                    'funcao ola(nome):',
                    '    escreva(f"Olá, eu me chamo {nome}")',
                    '    retorna nome != 10',
                    'escreva(tipo(ola("Victor")))'
                ];
                const retornoLexador = lexador.mapear(codigo, -1);
                const retornoAvaliadorSintatico = await avaliadorSintatico.analisar(retornoLexador, -1);
                const retornoInterpretador = await interpretador.interpretar(retornoAvaliadorSintatico.declaracoes);
                ...
                expect(retornoInterpretador.erros).toHaveLength(0);
                expect(_saidas).toHaveLength(2);
                expect(_saidas[0]).toBe('Olá, eu me chamo Victor');
                expect(_saidas[1]).toBe('lógico');
            });
        });
        ...
        describe('Métodos de primitivas com dependência no interpretador', () => {
           ...
           describe('Importação do módulo de testes', () => {
            ...
            it('Importar todos os elementos de testes com asterisco', async () => {
                const codigo = [
                    'de testes importar *',
                    'afirmar.igual(1, 1)',
            ...
            it('Importar elemento de testes com alias define variável global', async () => {
                const codigo = [
                    'de testes importar afirmar como a',
                    'a.igual(1, 1)'
                ];
                const retornoLexador = lexador.mapear(codigo, -1);
                const retornoAvaliadorSintatico = await avaliadorSintatico.analisar(retornoLexador, -1);
            ...
            it('Falha - Importação dinâmica', async () => {
                const codigo = [
                    't = importar("testes")'
                ];
                const retornoLexador = lexador.mapear(codigo, -1);
